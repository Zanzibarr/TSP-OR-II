...
void tsp_init_threads() {
    ...
    if (tsp_verbose == 5) tsp_print_info("Initializing threads.\n");
...
int tsp_wait_for_thread() {
    ...
    if (tsp_verbose == 5) {
        ...
        tsp_print_info("- %4d - Waiting for thread.\n", rnd_index);
    }

    while (1)
        for (int i = 0; i < N_THREADS; i++)
            if (tsp_available_threads[i]) {
                if (tsp_verbose == 5) tsp_print_info("- %4d - Thread %d available.\n", rnd_index, i);
...
void tsp_free_thread(const int t_index) {
    ...
    if (tsp_verbose == 5) tsp_print_info("Freeing thread %d.\n", t_index);
...
void tsp_wait_all_threads() {
    ...
    if (tsp_verbose == 5) tsp_print_info("Waiting for all threads to finish.\n");
    ...
    if (tsp_verbose == 5) tsp_print_info("All threads finished.\n");
