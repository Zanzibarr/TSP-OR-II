...
\begin{algorithm}[h]
    ...
    \begin{algorithmic}
        ...
        \EndFor
        \State $\mbox{cost}\gets\mbox{cost}+c(\mbox{cycle}[n-1],s)$\\\\
        \Return cycle, cost
    \end{algorithmic}
\end{algorithm}
\FloatBarrier
...
In general, we can define a \textit{k-opt} solution for the TPS problem as the solution obtained by changing $k$ edges to a pre-existing one. In this case, we implement a 2-opt algorithm that finds two edges that cross each other and fix them by removing the intersection:
