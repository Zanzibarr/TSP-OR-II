...
A first (naive) approach to the TSP, is to consider the edges with the \textbf{smallest weights first} when choosing the next node in the cycle.\\
...
If we had our cycle stored as a list of nodes ordered following the order of the nodes in the cycle, then this step can be done simply by \textbf{reversing the list} from the index $i+1$ to the index $j$:
$$[...,p_i,p_{i+1},...,p_j,p_{j+1},...]\Rightarrow[...,p_i,p_j,...(\mbox{reversed})...,p_{i+1},p_{j+1},...]$$
...
\end{enumerate}
Using a performance profiler, it's easy to see that the best\_swap policy consistently finds better solutions than the first\_swap policy:\\
... ADD A PERFORMANCE PROFILER FIRST\_SWAP VS BEST\_SWAP
